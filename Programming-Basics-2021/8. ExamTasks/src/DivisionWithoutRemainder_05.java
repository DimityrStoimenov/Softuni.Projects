...
public class DivisionWithoutRemainder_05 {
    public static void main(String[] args) {
        ...
        друг процент p2 се делят без остатък на 3, друг процент p3 се делят без остатък на 4. Да се напише програма, която
...
На следващите n-на брой реда стои по едно цяло число в интервала [1…1000] – числата които да бъдат проверени на колко се
делят.
Изход
Да се отпечатат на конзолата 3 реда, всеки от които съдържа процент между 0% и 100%, с точност две цифри след десетичната
точка, например 25.00%, 66.67%, 57.14%.
    ...
    • На третият ред – процентът на числата които се делят на 4
        ...
        int dividedByTwoCnt=0;
        int dividedByThreeCnt=0;
        int dividedByFourCnt=0;
        double percentTwo=0;
        ...
        percentTwo = (dividedByTwoCnt * 1.0 / n) *100;
        percentThree= (dividedByThreeCnt * 1.0 /n) *100;
        percentFour= ( dividedByFourCnt * 1.0 / n)* 100;
        System.out.printf("%.2f%%%n", percentTwo);
        System.out.printf("%.2f%%%n", percentThree);
        System.out.printf("%.2f%%", percentFour);
    }
}
